...
/mob/living/danimal/hostile/regalrat/Initialize()
	. = ..()
...
/mob/living/danimal/hostile/regalrat/proc/get_player()
...
/mob/living/danimal/hostile/regalrat/handle_automated_action()
	if(prob(20))
		riot.Trigger()
	else if(prob(50))
		coffer.Trigger()
...
/mob/living/danimal/hostile/regalrat/EvalTarget(atom/the_target)
	if(istype(the_target,/mob/living/danimal))
		var/mob/living/A = the_target
		if(istype(the_target, /mob/living/danimal/hostile/regalrat) && A.stat == CONSCIOUS)
			return TRUE
		if(istype(the_target, /mob/living/danimal/hostile/rat) && A.stat == CONSCIOUS)
			var/mob/living/danimal/hostile/rat/R = the_target
...
/mob/living/danimal/hostile/regalrat/examine(mob/user)
	. = ..()
	if(istype(user,/mob/living/danimal/hostile/rat))
		var/mob/living/danimal/hostile/rat/ratself = user
	...
	else if(istype(user,/mob/living/danimal/hostile/regalrat))
...
/datum/action/cooldown/riot/Trigger()
	...
	for(var/mob/living/danimal/mouse/M in oview(owner, 5))
		var/mob/living/danimal/hostile/rat/new_rat = new(get_turf(M))
		something_from_nothing = TRUE
		if(M.mind && M.stat == CONSCIOUS)
			M.mind.transfer_to(new_rat)
		if(istype(owner,/mob/living/danimal/hostile/regalrat))
			var/mob/living/danimal/hostile/regalrat/giantrat = owner
	...
	if(!something_from_nothing)
		if(LAZYLEN(SSmobs.cheeserats) >= cap)
			...
			return
		new /mob/living/danimal/mouse(owner.loc)
...
/mob/living/danimal/hostile/rat/Initialize(mapload)
	. = ..()
	if(cheesy)
...
/// nerfed rats for above-ground spawn
/// they like people
/mob/living/danimal/hostile/rat/skitter/curious
	name = "curious mouse"
...
/mob/living/danimal/hostile/rat/skitter/curious/Initialize()
	. = ..()
	emote("flip")
	emote("spin")
	emote("squeak")
...
/mob/living/danimal/hostile/rat/become_the_mob(mob/user)
...
/mob/living/danimal/hostile/rat/skitter/become_the_mob(mob/user)
	make_a_nest = /obj/effect/proc_holder/mob_common/make_nest/mouse
	. = ..()

/mob/living/danimal/hostile/rat/skitter/curious/become_the_mob(mob/user)
	call_backup = null
	send_mobs = null
	make_a_nest = null
	. = ..()
...
/mob/living/danimal/hostile/rat/frien/become_the_mob(mob/user)
	make_a_nest = /obj/effect/proc_holder/mob_common/make_nest/rat/tame
	. = ..()

/mob/living/danimal/hostile/rat/Destroy()
	if(cheesy)
		SSmobs.cheeserats -= src
	return ..()

/mob/living/danimal/hostile/rat/examine(mob/user)
	. = ..()
	if(istype(user,/mob/living/danimal/hostile/rat))
		var/mob/living/danimal/hostile/rat/ratself = user
		if(ratself.mob_faction_is_friendly_to_target(src, TRUE))
		...
		var/mob/living/danimal/hostile/regalrat/ratking = user
...
/mob/living/danimal/hostile/rat/EvalTarget(atom/the_target)
	if(istype(the_target,/mob/living/danimal))
		var/mob/living/A = the_target
		if(istype(the_target, /mob/living/danimal/hostile/regalrat) && A.stat == CONSCIOUS)
			var/mob/living/danimal/hostile/regalrat/ratking = the_target
			if(ratking.mob_faction_is_friendly_to_target(src, TRUE))
				return FALSE
			else
				return TRUE
...
/mob/living/danimal/hostile/rat/handle_automated_action()
	. = ..()
	if(prob(40))
...
/mob/living/danimal/hostile/rat/skitter/curious/frenly
	faction = list("neutral")


/mob/living/danimal/hostile/rat/skitter/curious/frenly/Initialize()
	. = ..()
