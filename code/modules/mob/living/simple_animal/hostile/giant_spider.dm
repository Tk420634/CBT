...
/mob/living/danimal/hostile/poison
	var/poison_per_bite = 5
	var/poison_type = /datum/reagent/toxin/rattler_venom

/mob/living/danimal/hostile/poison/AttackingTarget()
	. = ..()
...
/mob/living/danimal/hostile/poison/giant_spider/Initialize()
	. = ..()
	// lay_web = new
	// lay_web.Grant(src)

/mob/living/danimal/hostile/poison/giant_spider/Destroy()
	// QDEL_NULL(lay_web)
	return ..()

/mob/living/danimal/hostile/poison/giant_spider/Topic(href, href_list)
	if(href_list["activate"])
		var/mob/dead/observer/ghost = usr
		if(istype(ghost) && playable_spider)
			humanize_spider(ghost)

/mob/living/danimal/hostile/poison/giant_spider/Login()
	..()
	if(directive)
...
/mob/living/danimal/hostile/poison/giant_spider/attack_ghost(mob/user)
	. = ..()
	if(.)
		return
	humanize_spider(user)

/mob/living/danimal/hostile/poison/giant_spider/proc/humanize_spider(mob/user)
...
/mob/living/danimal/hostile/poison/giant_spider/emperor
	name = "emperor spider"
...
/mob/living/danimal/hostile/poison/giant_spider/empress
	name = "reaper spider"
...
/mob/living/danimal/hostile/poison/giant_spider/queen
	name = "queen spider"
...
/mob/living/danimal/hostile/poison/giant_spider/nurse
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/Initialize()
	. = ..()
	// wrap = new
	// AddAbility(wrap)
	// lay_eggs = new
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/Destroy()
	// RemoveAbility(wrap)
	// QDEL_NULL(lay_eggs)
...
/mob/living/danimal/hostile/poison/giant_spider/hunter/viper
...
/mob/living/danimal/hostile/poison/giant_spider/tarantula
	name = "radtarantula"
...
/mob/living/danimal/hostile/poison/giant_spider/tarantula/Moved(atom/oldloc, dir)
	. = ..()
	if(slowed_by_webs)
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/midwife
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/midwife/Initialize()
	. = ..()
	// letmetalkpls = new
	// letmetalkpls.Grant(src)

/mob/living/danimal/hostile/poison/giant_spider/nurse/midwife/Destroy()
...
/mob/living/danimal/hostile/poison/giant_spider/handle_automated_action()
	if(!..()) //AIStatus is off
		return 0
	if(AIStatus == AI_IDLE)
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/proc/GiveUp(C)
	return
	// spawn(100)
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/handle_automated_action()
	return ..()
	// if(..())
	...
	// 			if(C.stat && !istype(C, /mob/living/danimal/hostile/poison/giant_spider) && !C.anchored)
	// 				cocoon_target = C
...
/mob/living/danimal/hostile/poison/giant_spider/nurse/proc/cocoon()
	if(stat != DEAD && cocoon_target && !cocoon_target.anchored)
		if(cocoon_target == src)
			to_chat(src, span_warning("You can't wrap yourself!"))
			return
		if(istype(cocoon_target, /mob/living/danimal/hostile/poison/giant_spider))
...
/datum/action/innate/spider/lay_web/Activate()
	if(!istype(owner, /mob/living/danimal/hostile/poison/giant_spider))
		return
	var/mob/living/danimal/hostile/poison/giant_spider/S = owner
...
/obj/effect/proc_holder/wrap/InterceptClickOn(mob/living/the_one_who_called, params, atom/target)
	if(..())
		return
	if(ranged_ability_user.incapacitated() || !istype(ranged_ability_user, /mob/living/danimal/hostile/poison/giant_spider/nurse))
		remove_ranged_ability()
		return

	var/mob/living/danimal/hostile/poison/giant_spider/nurse/user = ranged_ability_user
	...
	if(user.Adjacent(target) && (ismob(target) || isobj(target)))
		...
		INVOKE_ASYNC(user, TYPE_PROC_REF(/mob/living/danimal/hostile/poison/giant_spider/nurse/,cocoon))
		remove_ranged_ability()
		return TRUE
...
/datum/action/innate/spider/lay_eggs/IsAvailable(silent = FALSE)
	if(..())
		if(!istype(owner, /mob/living/danimal/hostile/poison/giant_spider/nurse))
			return 0
		var/mob/living/danimal/hostile/poison/giant_spider/nurse/S = owner
		if(S.fed)
			return 1
		return 0
...
/datum/action/innate/spider/comm/IsAvailable(silent = FALSE)
	if(!istype(owner, /mob/living/danimal/hostile/poison/giant_spider/nurse/midwife))
		return FALSE
	return TRUE
...
/datum/action/innate/spider/comm/proc/spider_command(mob/living/user, message)
	...
	for(var/mob/living/danimal/hostile/poison/giant_spider/M in GLOB.spidermobs)
...
/*
/mob/living/danimal/hostile/poison/giant_spider/handle_temperature_damage()
