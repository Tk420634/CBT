...
/datum/click_interceptor/buildmode
	var/build_dir = SOUTH
	var/datum/click_interceptor/buildmode_mode/mode
	var/client/holder
...
/datum/click_interceptor/buildmode/New(client/c)
	mode = new /datum/click_interceptor/buildmode_mode/basic(src)
	holder = c
	buttons = list()
...
/datum/click_interceptor/buildmode/proc/post_login()
...
/datum/click_interceptor/buildmode/proc/create_buttons()
	...
	build_options_grid(subtypesof(/datum/click_interceptor/buildmode_mode), modeswitch_buttons, /atom/movable/screen/buildmode/modeswitch)
...
/datum/click_interceptor/buildmode/proc/build_options_grid(list/elements, list/buttonslist, buttontype)
	var/pos_idx = 0
...
/datum/click_interceptor/buildmode/proc/close_switchstates()
	switch(switch_state)
		if(BM_SWITCHSTATE_MODE)
			close_modeswitch()
		if(BM_SWITCHSTATE_DIR)
...
/datum/click_interceptor/buildmode/proc/toggle_modeswitch()
...
/datum/click_interceptor/buildmode/proc/open_modeswitch()
	switch_state = BM_SWITCHSTATE_MODE
	holder.screen += modeswitch_buttons

/datum/click_interceptor/buildmode/proc/close_modeswitch()
	switch_state = BM_SWITCHSTATE_NONE
	holder.screen -= modeswitch_buttons

/datum/click_interceptor/buildmode/proc/toggle_dirswitch()
...
/datum/click_interceptor/buildmode/proc/open_dirswitch()
	switch_state = BM_SWITCHSTATE_DIR
	holder.screen += dirswitch_buttons

/datum/click_interceptor/buildmode/proc/close_dirswitch()
	switch_state = BM_SWITCHSTATE_NONE
	holder.screen -= dirswitch_buttons

/datum/click_interceptor/buildmode/proc/change_mode(newmode)
	mode.exit_mode(src)
	QDEL_NULL(mode)
	close_switchstates()
...
/datum/click_interceptor/buildmode/proc/change_dir(newdir)
	build_dir = newdir
	close_dirswitch()
	dirbutton.update_icon()
	return 1

/datum/click_interceptor/buildmode/InterceptClickOn(mob/user, params, atom/object)
