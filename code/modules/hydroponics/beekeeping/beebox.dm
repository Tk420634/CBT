...
/mob/living/danimal/hostile/poison/bees/bee_friendly()
	return 1
...
/obj/structure/beebox
	...
	anchored = TRUE
	density = TRUE
	var/mob/living/danimal/hostile/poison/bees/queen/queen_bee = null
...
/obj/structure/beebox/premade/Initialize()
	...
	for(var/i in 1 to get_max_bees())
		var/mob/living/danimal/hostile/poison/bees/B = new(src)
		bees += B
		B.beehome = src
		B.assign_reagent(R)
	...
	if(queen_bee)
		...
		if(bees.len < get_max_bees())
			...
			if((bee_resources >= BEE_RESOURCE_NEW_BEE_COST && prob(BEE_PROB_NEW_BEE)) || freebee)
				...
				var/mob/living/danimal/hostile/poison/bees/B = new(get_turf(src))
				B.beehome = src
...
/obj/structure/beebox/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/queen_bee))
		...
		if(queen_bee)
			...
			var/relocated = 0
			for(var/b in bees)
				var/mob/living/danimal/hostile/poison/bees/B = b
...
/obj/structure/beebox/deconstruct(disassembled = TRUE)
	...
	for(var/mob/living/danimal/hostile/poison/bees/B in bees)
		if(B.loc == src)
			B.forceMove(drop_location())
