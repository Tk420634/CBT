...
/datum/round_event_control/wizard/petsplosion/preRunEvent()
	for(var/mob/living/danimal/F in GLOB.alive_mob_list)
...
/datum/round_event/wizard/petsplosion/tick()
	if(activeFor >= 30 * countdown) // 0 seconds : 2 animals | 30 seconds : 4 animals | 1 minute : 8 animals
		countdown += 1
		for(var/mob/living/danimal/F in GLOB.alive_mob_list) //If you cull the heard before the next replication, things will be easier for you
			if(!ishostile(F) && is_station_level(F.z))
				new F.type(F.loc)
				mobs_duped++
				if(mobs_duped > 400)
