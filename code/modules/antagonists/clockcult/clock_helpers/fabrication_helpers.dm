...
/mob/living/danimal/fabricator_heal_tick(amount)
	adjustHealth(-amount)
...
/mob/living/danimal/hostile/clockwork/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	. = TRUE
	if(health == maxHealth) //if we're at maximum health, replace the turf under us
		return FALSE
	else if(fabricator_heal(user, fabricator) && user)
...
/mob/living/danimal/drone/cogscarab/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	. = TRUE
	if(stat == DEAD)
