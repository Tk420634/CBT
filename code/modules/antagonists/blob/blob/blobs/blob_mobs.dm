...
//Do not spawn
/mob/living/danimal/hostile/blob
...
/mob/living/danimal/hostile/blob/update_icons()
	if(overmind)
		add_atom_colour(overmind.blobstrain.color, FIXED_COLOUR_PRIORITY)
	else
		remove_atom_colour(FIXED_COLOUR_PRIORITY)

/mob/living/danimal/hostile/blob/Destroy()
	if(overmind)
		overmind.blob_mobs -= src
	return ..()

/mob/living/danimal/hostile/blob/blob_act(obj/structure/blob/B)
...
/mob/living/danimal/hostile/blob/fire_act(exposed_temperature, exposed_volume)
	..()
...
/mob/living/danimal/hostile/blob/CanAllowThrough(atom/movable/mover, border_dir)
	..()
	if(istype(mover, /obj/structure/blob))
		return 1
	return ..()

/mob/living/danimal/hostile/blob/Process_Spacemove(movement_dir = 0, continuous_move)
	for(var/obj/structure/blob/B in range(1, src))
		return 1
	return ..()

/mob/living/danimal/hostile/blob/proc/blob_chat(msg)
	var/spanned_message = say_quote(msg)
	var/rendered = "<font color=\"#EE4000\"><b>\[Blob Telepathy\] [real_name]</b> [spanned_message]</font>"
	for(var/M in GLOB.mob_list)
		if(isovermind(M) || istype(M, /mob/living/danimal/hostile/blob))
			to_chat(M, rendered)
		if(isobserver(M))
...
/mob/living/danimal/hostile/blob/blobspore
	name = "blob spore"
...
/mob/living/danimal/hostile/blob/blobspore/Initialize(mapload, obj/structure/blob/factory/linked_node)
	if(istype(linked_node))
		factory = linked_node
		factory.spores += src
	. = ..()

/mob/living/danimal/hostile/blob/blobspore/BiologicalLife(seconds, times_fired)
	if(!(. = ..()))
		return
...
/mob/living/danimal/hostile/blob/blobspore/proc/Zombify(mob/living/carbon/human/H)
	is_zombie = 1
	if(H.wear_suit)
...
/mob/living/danimal/hostile/blob/blobspore/death(gibbed)
...
/mob/living/danimal/hostile/blob/blobspore/Destroy()
	if(factory)
		factory.spores -= src
	factory = null
	if(oldguy)
...
/mob/living/danimal/hostile/blob/blobspore/update_icons()
	if(overmind)
...
/mob/living/danimal/hostile/blob/blobspore/weak
...
/mob/living/danimal/hostile/blob/blobbernaut
	name = "blobbernaut"
...
/mob/living/danimal/hostile/blob/blobbernaut/Initialize()
	. = ..()
	if(independent)
		pass_flags &= ~PASSBLOB

/mob/living/danimal/hostile/blob/blobbernaut/start_pulling(atom/movable/AM, state, force = pull_force, supress_message = FALSE)
...
/mob/living/danimal/hostile/blob/blobbernaut/BiologicalLife(seconds, times_fired)
	if(!(. = ..()))
		return
...
/mob/living/danimal/hostile/blob/blobbernaut/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	. = ..()
	if(updating_health)
		update_health_hud()

/mob/living/danimal/hostile/blob/blobbernaut/update_health_hud()
	if(hud_used)
...
/mob/living/danimal/hostile/blob/blobbernaut/update_icons()
	..()
...
/mob/living/danimal/hostile/blob/blobbernaut/death(gibbed)
	..(gibbed)
	if(factory)
